...
def should_skip_path(path, excluded_dirs, excluded_files):
    """
    ...
    Args:
        ...
        excluded_files (set): Set of file names to exclude
    ...
    # Check if file name is in excluded_files
...
def read_files_to_string(folder_path_str, recursive=True, extensions=None, max_size=None, 
    ...
    print(f"Excluding directories: {', '.join(excluded_dirs)}", file=sys.stderr)
    print(f"Excluding files: {', '.join(excluded_files)}", file=sys.stderr)
    ...
    for item_path in folder_path.glob(glob_pattern):
        if item_path.is_file():
            ...
            if should_skip_path(item_path, excluded_dirs, excluded_files):
    ...
    summary = (
        ...
        f"Skipped (Excluded): {skipped_excluded_count} files.\n"
        f"Total items considered in '{folder_path.name}': {found_files_count + skipped_extension_count + skipped_size_count + skipped_read_error_count + skipped_excluded_count}\n"
        f"--- End Summary ---\n"
    )
